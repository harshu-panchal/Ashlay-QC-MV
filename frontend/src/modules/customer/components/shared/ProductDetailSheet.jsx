...
import { X, ChevronDown, Share2, Heart, Search, Clock, Minus, Plus, ShoppingBag, Star, MessageSquare, ArrowLeft, ChevronRight, ChevronLeft, ShieldCheck, RotateCcw, Truck, Award, Check, ThumbsUp, ThumbsDown } from 'lucide-react';
...
const getColorHex = (name) => {
    ...
    if (lower.includes("black")) return "#0F0F11";
    if (lower.includes("blue") || lower.includes("navy")) return "#0A2540";
    if (lower.includes("gold") || lower.includes("wheat") || lower.includes("tan") || lower.includes("beige")) return "#D4AF37";
    if (lower.includes("grey") || lower.includes("gray") || lower.includes("silver")) return "#B3B3B3";
...
const ProductDetailSheet = () => {
    ...
    const colorOptions = useMemo(() => {
        const backendColors = selectedProduct?.colorOptions;
        if (Array.isArray(backendColors) && backendColors.length > 0) {
            return backendColors.map((color, index) => ({
                id: color.id || color._id || `color-${index}`,
        ...
        // Mock fallback until backend provides colorOptions
        return [
            { id: "black", name: "Black", hex: "#0F0F11", isAvailable: true },
            { id: "navy", name: "Navy", hex: "#0A2540", isAvailable: true },
            { id: "sand", name: "Sand", hex: "#D4AF7A", isAvailable: true },
            { id: "ash", name: "Ash", hex: "#C4C4C4", isAvailable: true },
        ];
    }, [selectedProduct]);
    ...
    const detailsData = useMemo(() => {
        ...
        return [
            ...
            selectedProduct.weight && { label: 'Weight / Pack Size', value: selectedProduct.weight, emoji: '⚖️' },
            selectedProduct.sku && { label: 'Product Code (SKU)', value: selectedProduct.sku, emoji: '🔢' }
        ].filter(Boolean);
    }, [selectedProduct]);
    ...
    const averageRatingValue = reviewCount > 0
        ? reviews.reduce((acc, r) => acc + (Number(r.rating) || 0), 0) / reviewCount
        : 0;
    const filledRatingStars = Math.round(averageRatingValue);
            ...
            {isOpen && (
                <>
                    ...
                    <motion.div
                        ...
                        className="hidden md:flex fixed z-[230] top-[72px] bottom-[16px] left-[3%] right-[3%] lg:left-[6%] lg:right-[6%] xl:left-[12%] xl:right-[12%] bg-white rounded-3xl shadow-[0_40px_100px_rgba(0,0,0,0.25)] overflow-hidden font-inter product-detail-sheet"
                                                ...
                                                <div className="grid grid-cols-2 gap-3 mt-1">
                                                    {detailsData.map((d) => (
                                                        <div key={d.label} className="bg-slate-50 p-2.5 rounded-xl border border-slate-100 group hover:bg-white hover:shadow-sm transition-all">
                                                            <span className="text-[10px] text-slate-400 block mb-0.5 font-bold uppercase tracking-wider">{d.label} {d.emoji}</span>
                    ...
                    <motion.div
                        ...
                        className={cn(
                            "md:hidden fixed z-[230] bg-white shadow-2xl overflow-hidden flex flex-col font-inter product-detail-sheet",
                        )}
                        style={{
                            willChange: "transform, top, bottom, left, width, border-radius",
                            fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif"
                        }}
                    >
                        {/* Header Bar */}
                        <div className="bg-white border-b border-slate-100 px-4 py-2.5 flex items-center justify-between z-40 select-none flex-shrink-0">
                            <motion.button
                                onClick={closeProduct}
                                whileTap={{ scale: 0.95 }}
                                className="w-10 h-10 flex items-center justify-start text-slate-800 active:opacity-60 transition-opacity"
                            >
                                <ChevronLeft size={26} strokeWidth={2.5} />
                            </motion.button>

                            <h2
                                className="text-[17px] font-[800] text-slate-900 tracking-tight"
                            ...
                            <motion.button
                                ...
                                whileTap={{ scale: 0.95 }}
                                className="w-10 h-10 flex items-center justify-end text-slate-800 active:opacity-60 transition-opacity"
                            >
                        ...
                        >
                            ...
                            <div className="px-4 pt-4 flex-shrink-0">
                                <div className="relative w-full bg-[#F5F5F7] rounded-[24px] overflow-hidden h-[38vh] min-h-[260px] max-h-[440px] flex items-center justify-center border border-slate-100">
                                    <div
                                        ref={scrollRef}
                                    ...
                                    >
                                        {allImages.map((img, i) => (
                                            <div key={i} className="flex-shrink-0 w-full h-full snap-center flex items-center justify-center p-6">
                                                <motion.img
                                    ...
                                    {/* Wishlist Button floating in Top-Right corner */}
                                    <motion.button
                                        onClick={() => toggleWishlistGlobal(selectedProduct)}
                                        whileTap={{ scale: 0.9 }}
                                        className="absolute top-4 right-4 w-11 h-11 bg-white shadow-md rounded-full flex items-center justify-center z-20 border border-slate-100/50"
                                    >
                                        <Heart
                                            size={20}
                                            className={isInWishlist(selectedProduct.id) ? "text-red-500 fill-red-500" : "text-slate-800"}
                                            strokeWidth={2}
                                        />
                                    </motion.button>
                                            ...
                                            {allImages.map((_, i) => (
                                                <div
                                                    ...
                                                    className={cn(
                                                        ...
                                                        i === activeImageIndex ? "w-6 bg-slate-800" : "w-1.5 bg-slate-300"
                                                    )}
                                                />
                                            ))}
                                        </div>
                            ...
                            <div className="px-5 pt-5 pb-6">
                                {/* Product Name */}
                                <h1 className="text-xl font-[800] text-slate-900 leading-tight tracking-tight mb-1.5">
                                    {selectedProduct.name}
                                </h1>
                                    ...
                                    <div className="flex gap-0.5 text-amber-400">
                                        {[...Array(5)].map((_, i) => (
                                            <Star
                                                ...
                                                size={15}
                                                className={cn(
                                                    i < filledRatingStars ? "text-amber-400 fill-current" : "text-slate-200"
                                                )}
                                            />
                                        ))}
                                    </div>
                                    <span className="text-[12px] font-[600] text-slate-500">
                                ...
                                <div className="flex items-baseline gap-3 mb-5">
                                    <span className="text-[22px] font-[900] text-slate-900 leading-none">
                                    ...
                                    {((selectedVariant?.salePrice && selectedVariant.salePrice < selectedVariant.price) ||
                                        (!selectedVariant && selectedProduct.originalPrice > selectedProduct.price)) && (
                                            <>
                                                <span className="text-[15px] font-[600] text-slate-400 line-through">
                                ...
                                {colorOptions.length > 0 && (
                                    <div className="mb-6">
                                        <h3 className="text-[13px] font-[800] text-slate-800 tracking-tight mb-3">
                                            Select Color
                                        </h3>
                                            ...
                                            {colorOptions.map((color) => {
                                                ...
                                                return (
                                                    <motion.button
                                                        ...
                                                        className={cn(
                                                            "relative w-9 h-9 rounded-full transition-all focus:outline-none flex items-center justify-center",
                                                            !color.isAvailable && "opacity-50 cursor-not-allowed"
                                                        )}
                                                        style={{
                                                            border: isSelected ? "2px solid #0F0F11" : "1px solid #E2E8F0",
                                                            padding: "2px",
                                                        }}
                                ...
                                {selectedProduct.variants && selectedProduct.variants.length > 0 && (
                                    <div className="mb-6">
                                        <h3 className="text-[13px] font-[800] text-slate-800 tracking-tight mb-3">
                                            {selectedProduct.variants[0]?.name?.toLowerCase()?.includes("color") ||
                                                getColorHex(selectedProduct.variants[0]?.name) !== null ? "Select Color" : "Select Variant"}
                                        </h3>
                                        <div className="flex flex-wrap gap-3">
                                            {selectedProduct.variants.map((v, idx) => {
                                                const isSelected = selectedVariant?.sku === v.sku;
                                                const hexColor = getColorHex(v.name);
                                                        ...
                                                        >
                                                            <div
                                                                ...
                                                                style={{
                                                                    ...
                                                                    border: hexColor === '#FFFFFF' ? '1px solid #CBD5E1' : 'none'
                                                                }}
                                                            />
                                                        </motion.button>
                                                    );
                                ...
                                {/* Key Features / Description from DB */}
                                {cleanDesc && (
                                    <div className="mb-6 pt-2">
                                        <div
                                            className="text-[13px] text-slate-600 font-normal leading-relaxed edit-rich-text"
                                            dangerouslySetInnerHTML={{ __html: cleanDesc }}
                                        />
                                    </div>
                                )}

                                {/* Customer Reviews Section (Flat, below Key Features) */}
                                {(() => {
                                    ...
                                    const renderStars = (rating) => {
                                        return [...Array(5)].map((_, i) => {
                                            const isFull = i < Math.floor(Number(rating));
                                            return (
                                                <Star
                                                    key={i}
                                                    size={13}
                                                    fill={isFull ? "#f59e0b" : "none"}
                                                    className={isFull ? "text-amber-500 fill-amber-500" : "text-slate-200"}
                                                />
                                            );
                                        });
                                    };
                                    return (
                                        <div className="mt-8 pt-6 border-t border-slate-100">
                                            ...
                                            <div className="flex items-center justify-between mb-5 select-none">
                                                <h3 className="text-[15px] font-[800] text-[#061939] tracking-tight">Customer Reviews</h3>
                                            </div>

                                            {reviews.length > 0 ? (
                                                <>
                                                    {/* Premium Distribution Card */}
                                                    <div className="flex items-center gap-5 mb-6 bg-slate-50/50 p-4 rounded-2xl border border-slate-100/60">
                                                        {/* Left side: Avg Rating */}
                                                        <div className="flex flex-col items-center justify-center border-r border-slate-100 pr-5 flex-shrink-0 select-none">
                                                            <span className="text-[42px] font-[900] text-[#061939] leading-none mb-1">
                                                                {averageRating}
                                                            </span>
                                                            ...
                                                            {[5, 4, 3, 2, 1].map((star) => {
                                                                ...
                                                                return (
                                                                    <div key={star} className="flex items-center gap-2 text-[10px]">
                                                                        <span className="w-2 font-[800] text-slate-600 text-right">{star}</span>
                                                                        <Star size={8} className="text-slate-400 fill-current" />
                                                                        ...
                                                                        <span className="w-5 text-right font-[600] text-slate-400">{count}</span>
                                                                    </div>
                                                                );
                                                            })}
                                                        </div>
                                                    ...
                                                    <div className="space-y-4 mb-6">
                                                        {reviews.slice(0, 3).map((r) => {
                                                            const initials = r.userId?.name
                                                                ? r.userId.name.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase()
                                                                : 'AR';
                                                            return (
                                                                <div key={r._id || r.id} className="p-4 rounded-2xl border border-slate-100 bg-white hover:shadow-sm transition-all relative">
                                                                    <div className="flex justify-between items-start mb-3">
                                                                        <div className="flex items-center gap-3">
                                                                            {/* Avatar circle */}
                                                                            <div className="h-9 w-9 rounded-full bg-slate-50 flex items-center justify-center text-[11px] font-[800] text-[#061939] border border-slate-200 flex-shrink-0">
                                                                                {initials}
                                                                            </div>
                                                                            <div>
                                                                                <div className="flex items-center gap-1.5 flex-wrap">
                                                                                    <p className="text-[12px] font-[800] text-[#061939]">{r.userId?.name || 'Anonymous'}</p>
                                                                                    {/* Verified Buyer Badge */}
                                                                                    <span className="flex items-center gap-0.5 bg-[#0A1931] text-white text-[8px] font-[700] px-1.5 py-0.5 rounded-full select-none">
                                                                                        <Check size={7} strokeWidth={3.5} />
                                                                                        Verified Buyer
                                                                                    </span>
                                                                                </div>
                                                                                <span className="text-[9px] font-[600] text-slate-400 block mt-0.5">
                                                                                    {new Date(r.createdAt || Date.now()).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}
                                                                                </span>
                                                                            </div>
                                                                        </div>

                                                                        {/* Star Ratings on top-right */}
                                                                        <div className="flex gap-0.5 text-amber-500">
                                                                            {renderStars(r.rating)}
                                                                        </div>
                                                                    </div>

                                                                    {/* Review comment Title & Body */}
                                                                    <div className="pl-1 leading-relaxed mb-2.5">
                                                                        <h4 className="text-[12px] font-[800] text-[#061939] mb-1">
                                                                            {r.rating >= 4 ? 'Excellent Quality!' : r.rating === 3 ? 'Good product' : 'Decent purchase'}
                                                                        </h4>
                                                                    ...
                                                                    {/* Helpful buttons */}
                                                                    <div className="flex justify-end gap-2.5 text-[9px] text-slate-400 select-none">
                                                                        <span className="font-[600] self-center">Helpful?</span>
                                                                        <button className="flex items-center gap-1 px-2 py-0.5 bg-slate-50 hover:bg-slate-100 active:scale-95 transition-all rounded-full border border-slate-100">
                                                                            <ThumbsUp size={9} className="text-slate-400" />
                                                                            <span>24</span>
                                                                        </button>
                                                                        <button className="flex items-center gap-1 px-2 py-0.5 bg-slate-50 hover:bg-slate-100 active:scale-95 transition-all rounded-full border border-slate-100">
                                                                            <ThumbsDown size={9} className="text-slate-400" />
                                                                            <span>2</span>
                                                                        </button>
                                                                    </div>
                                                                </div>
                                                ...
                                                /* If no reviews yet */
                                                <div className="py-10 text-center bg-slate-50/50 rounded-2xl border border-dashed border-slate-200 mb-6 select-none">
                                                    <MessageSquare size={22} className="text-slate-300 mx-auto mb-2.5" />
                                                    <p className="text-[12px] font-[800] text-slate-400 uppercase tracking-widest">No reviews yet</p>
                                                </div>
                                            )}

                                            {/* Modern Premium Review Submission Form */}
                                            <div className="bg-slate-50/80 p-5 rounded-2xl border border-slate-100 mb-2">
                                                <h4 className="font-[800] text-[#061939] text-[10px] uppercase tracking-wider mb-3">
                                                    {reviews.length > 0 ? "Add a Review" : "Be the first to add a review"}
                                                </h4>
                                                <form onSubmit={handleReviewSubmit} className="space-y-4">
                                                    ...
                                                    <div className="flex gap-2">
                                                        {[1, 2, 3, 4, 5].map((s) => (
                                                            <button
                                                                ...
                                                                onClick={() => setNewReview({ ...newReview, rating: s })}
                                                                className={cn(
                                                                    "h-9 w-9 rounded-xl flex items-center justify-center transition-all shadow-sm border",
                                                                    newReview.rating >= s
                                                    ...
                                                    <textarea
                                                        ...
                                                        placeholder={reviews.length > 0 ? "Share your experience with this product..." : "Be the first to add a review..."}
                                                        className="w-full bg-white border border-slate-100 rounded-2xl p-4 text-[12px] font-normal min-h-[90px] outline-none focus:border-slate-300 transition-all resize-none shadow-sm text-[#061939]"
                                                    />
                                                    ...
                                                    <Button
                                                        ...
                                                        className="w-full h-11 bg-[#0A1931] hover:opacity-90 text-white font-[800] rounded-xl text-xs uppercase tracking-widest transition-all shadow-md border-none"
                                                    >
                        ...
                        <div className="absolute bottom-0 left-0 right-0 bg-white border-t border-gray-100 p-4 pb-6 shadow-[0_-10px_40px_rgba(0,0,0,0.05)] z-50">
                            <div className="flex flex-col gap-3">
                                <div className="flex items-center gap-3 w-full">
                                    {/* Add to Cart Button */}
                                    {quantity > 0 ? (
                                        <div className="flex items-center justify-between border border-slate-200 rounded-xl h-[48px] px-2 flex-1 max-w-[50%] bg-white select-none">
                                            <motion.button
                                                whileTap={{ scale: 0.9 }}
                                                onClick={handleDecrement}
                                                className="w-8 h-8 rounded-lg flex items-center justify-center text-slate-500 active:bg-slate-50"
                                            >
                                                <Minus size={16} strokeWidth={3} />
                                            </motion.button>
                                            <span className="font-[800] text-slate-800 text-base w-8 text-center tabular-nums">{quantity}</span>
                                            <motion.button
                                                whileTap={{ scale: 0.9 }}
                                        ...
                                        <motion.button
                                            whileTap={{ scale: 0.95 }}
                                            onClick={handleAddToCart}
                                            className="flex-1 border border-slate-300 hover:border-slate-400 bg-white text-slate-800 h-[48px] rounded-xl font-[700] text-sm flex items-center justify-center gap-2 transition-all font-inter"
                                        >
                                    ...
                                    <motion.button
                                        ...
                                        className="flex-1 bg-[#0A1931] hover:bg-[#081528] text-white h-[48px] rounded-xl font-[700] text-sm flex items-center justify-center transition-all shadow-md shadow-[#0A1931]/10 font-inter"
                                    >
                                        Buy Now
                                    </motion.button>
                                </div>
